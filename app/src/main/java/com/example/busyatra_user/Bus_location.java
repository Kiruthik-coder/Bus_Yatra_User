...
public class Bus_location extends FragmentActivity implements OnMapReadyCallback {
    ...
    private static final int RADIUS_IN_METERS = 2000;
    ...
    private BitmapDescriptor createCustomIcon(String busName) {
        ...
        // Adjust the canvas for vertical centering of text
        Paint.FontMetrics fontMetrics = textPaint.getFontMetrics();
        float textHeight = fontMetrics.descent - fontMetrics.ascent;
        float y = (canvas.getHeight() - textHeight) / 2 - fontMetrics.ascent;

        // Wrap the text to fit within the marker width
        int maxWidth = bitmap.getWidth() - 20; // 20 padding on each side
        StaticLayout staticLayout = new StaticLayout(busName, textPaint, maxWidth, Layout.Alignment.ALIGN_CENTER, 1.0f, 0.0f, false);

        // Draw the text on the marker
        canvas.save();
        canvas.translate(10, y); // Adding 10 padding on the left side
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        LatLng southwestLatLng = new LatLng(12.9692, 79.1559);
        LatLng northeastLatLng = new LatLng(12.9698,79.1565);
        LatLngBounds bounds = new LatLngBounds(southwestLatLng, northeastLatLng);

        // Set the visible region on the map
        mMap.setLatLngBoundsForCameraTarget(bounds);
        mMap.setMinZoomPreference(13.5f);

        // Move the camera to VIT Vellore's location and set an appropriate zoom level
        //mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(vitVelloreLatLng, 16.0f));
    ...
    private void addTransparentCircleOverlay(LatLng center, double radiusInMeters) {
        CircleOptions circleOptions = new CircleOptions()
                .center(center)
                .radius(radiusInMeters)
                .fillColor(Color.TRANSPARENT) // Transparent fill color
                .strokeColor(Color.TRANSPARENT) // Transparent stroke color
                .strokeWidth(0); // No stroke width

        mMap.addCircle(circleOptions);
    }
    private void addSolidColorOverlay(LatLng center, double radiusInMeters) {
        CircleOptions circleOptions = new CircleOptions()
                .center(center)
                .radius(radiusInMeters)
                .fillColor(Color.parseColor("#80FFFFFF")) // Replace with the desired color and transparency
                .strokeColor(Color.TRANSPARENT) // Transparent stroke color
                .strokeWidth(0); // No stroke width
